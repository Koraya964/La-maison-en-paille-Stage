  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/dashboard/realisations/nouvelle" className="btn-primary">+ Uploader une photo</Link>
      </div>
      ...
      {realisations.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {realisations.map((r) => (
            <Link key={r.id} href={`/dashboard/realisations/${r.id}`} className="group relative block">
              ...
              <div className="absolute inset-0 bg-[#3d2b1f] opacity-0 group-hover:opacity-60 transition-opacity flex items-center justify-center">
                <span className="text-white text-xs tracking-widest uppercase font-bold">Modifier</span>
              </div>
              {r.titre && (
                <p className="text-xs text-stone-600 mt-1 truncate">{r.titre}</p>
              )}
            </Link>
          ))}
        </div>
