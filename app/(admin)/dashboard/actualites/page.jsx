...
async function getActualites() {
  try {
    return await query('SELECT * FROM actualites ORDER BY created_at DESC')
  } catch {
    return []
  }
}
        ...
        {actualites.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-50 border-b border-stone-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs tracking-widest uppercase text-stone-500 font-bold">Titre</th>
                <th className="px-6 py-4 text-left text-xs tracking-widest uppercase text-stone-500 font-bold">Statut</th>
                <th className="px-6 py-4 text-left text-xs tracking-widest uppercase text-stone-500 font-bold">Date</th>
              ...
              {actualites.map((news) => (
                <tr key={news.id} className="hover:bg-stone-50 transition-colors">
                  <td className="px-6 py-4">
                    <p className="font-medium text-[#3d2b1f] truncate max-w-xs">{news.titre}</p>
                    <p className="text-stone-400 text-xs mt-0.5 truncate max-w-xs">
                      {news.contenu?.slice(0, 80)}…
                    </p>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-block text-xs tracking-wider uppercase px-2 py-1 ${
                      news.publie
                  ...
                  <td className="px-6 py-4 text-stone-500">{formatDate(news.created_at)}</td>
