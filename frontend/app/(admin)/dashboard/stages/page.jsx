    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-stone-500 mt-1">{stages.length} session(s) programmée(s)</p>
        </div>
        <Link href="/dashboard/stages/nouveau" className="btn-primary">+ Nouveau stage</Link>
      </div>

      <div className="bg-white border border-stone-200 rounded-lg overflow-hidden">
        {stages.length === 0 ? (
          <div className="p-16 text-center text-stone-400 font-serif text-lg">Aucun stage programmé.</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-stone-50 border-b border-stone-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs tracking-widest uppercase text-stone-500 font-bold">Formation</th>
                <th className="px-6 py-4 text-left text-xs tracking-widest uppercase text-stone-500 font-bold">Dates</th>
                <th className="px-6 py-4 text-left text-xs tracking-widest uppercase text-stone-500 font-bold">Places</th>
            ...
            <tbody className="divide-y divide-stone-100">
              {stages.map((stage) => (
                <tr key={stage.id} className="hover:bg-stone-50 transition-colors">
                  <td className="px-6 py-4 font-medium text-[#3d2b1f]">{stage.formation_titre}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-block text-xs tracking-wider uppercase px-2 py-1 ${statutColors[stage.statut]}`}>
