...
const IMAGES = {
  ...
  poele:
    "https://static.wixstatic.com/media/3e33e8_06a3e5044e5143f5932fcc4f69a2eede~mv2.jpg/v1/fill/w_941,h_706,al_c,q_85,usm_0.66_1.00_0.01,enc_auto/3e33e8_06a3e5044e5143f5932fcc4f69a2eede~mv2.jpg",
  photo:
...
function BarrePlaces({ places, total = 10, statut }) {
  const pct =
    statut === "complet" || places === 0
      ? 100
      : Math.round((places / total) * 100);
  const color =
    statut === "complet" || places === 0
      ? "#7f77dd"
      : places <= 3
...
function DateRow({ stage, accent, slug }) {
  return (
    <div
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <span
          ...
          style={{ color: "#9a8070" }}
        >
          {stage.statut === "complet" || stage.places_dispo === 0
            ? "Complet"
...
function ProchainsDates({ formations }) {
  const rows = formations.map((f) => {
    ...
    const stages = (f.stages || []).filter(
      (s) => s.statut === "ouvert" || s.statut === "complet",
    );
    return { ...f, accent: config.accent || "#8b5e3c", stages };
  });
}
    ...
    <div style={{ backgroundColor: BEIGE }}>
      ...
      >
        ...
        <div className="relative z-10 text-center px-6 flex flex-col items-center gap-8 max-w-3xl">
          ...
          <div
            className={`mt-4 flex flex-col items-center gap-2 transition-opacity duration-500 ${scrolled ? "opacity-0 pointer-events-none" : "opacity-100"}`}
            aria-hidden="true"
          >
      ...
      {/* ══ PROCHAINES DATES ══ */}
      <ProchainsDates formations={formations} />
